...
class Polygon {
    ...
    boolean isConvex() {
        if (g.length <= 3) // point or line
            return false;
        boolean ccw = Point.ccw(g[g.length - 2], g[0], g[1]); // edit ccw check
        // to accept
        // collinear
        // points
    ...
    boolean inside(Point p) // for convex/concave polygons - winding number
    // algorithm
    {
        double sum = 0.0;
        for (int i = 0; i < g.length - 1; ++i) {
        ...
        return Math.abs(2 * Math.PI - Math.abs(sum)) < EPS; // abs makes it work
        // for clockwise
    }

	/*
	 * Another way if the polygon is convex 1. Triangulate the poylgon through p
	 * 2. Check if sum areas == polygon area 3. Handle empty polygon
	 */

    Polygon cutPolygon(Point a, Point b) // returns the left part of the
    // polygon, swap a & b for the right
    // part
    {
        ...
        if (size != 0 && ans[0] != ans[size - 1]) // necessary in case g[0] is
            // not in the new polygon
            ans[size++] = ans[0];
        return new Polygon(Arrays.copyOf(ans, size));
    }

    static Polygon convexHull(Point[] points) // all points are unique, remove
    // duplicates, edit ccw to
...
class Circle { // equation: (x-c.x)^2 + (y-c.y)^2 = r^2
    ...
    // returns true if the circle intersects with the line segment defined by p
    // and q at one or two points
    ...
    static Point findCenter(Point p, Point q, double r) // for the other center,
    // swap p and q
    {
...
class Geometry {
    ...
    static final double INF = 1e9, EPS = 1e-9; // better use 1e-11 for large
    // coordinates and 1e-15 if
    // infinite precision is
    // required
...
class Triangle {
    ...
    double area() {
        ...
        return Math.sqrt(s * (s - ab) * (s - bc) * (s - ca)); // take care of
        // overflow
    }
	 ...
	 * means that point q is on the left of the line segment pr
	 *
    ...
    Point inCenter() // intersection of the angle bisectors, center of inscribed
    // circle
    {
        double p = perm();
    ...
    static double areaMedians(double ma, double mb, double mc) // medians of the
    // triangle
    {
    ...
    static double areaHeights(double ha, double hb, double hc) // heights of the
    // triangle
    {
